...
#include "helpers.h"
#include "render.h"
#include "rw/d3d8vertexshaderutils.h"
#include "rw/lightfinder.h"
#ifdef LOADSHADERS
...
VertexShaderDeclarationArray[][32] =
{
	...
	// 4 rxGEOMETRY_MODULATE
	#include "vsdecl.def"
	,

	// 5 rxGEOMETRY_MODULATE | rxGEOMETRY_PRELIT
	#define  WITHDIFFUSE
	#include "vsdecl.def"
	,
	#undef   WITHDIFFUSE

	// 6 rxGEOMETRY_MODULATE | rxGEOMETRY_NORMALS
	#define  WITHNORMAL
	#include "vsdecl.def"
	,
	#undef   WITHNORMAL

	// 7 rxGEOMETRY_MODULATE | rxGEOMETRY_PRELIT | rxGEOMETRY_NORMALS
	#define  WITHNORMAL
	#define  WITHDIFFUSE
	#include "vsdecl.def"
	#undef   WITHDIFFUSE
...
#define  dwVertexShader dwVertexShaderVS4
#include "shaders\VS4.h"
#undef   dwVertexShader

#define  dwVertexShader dwVertexShaderVS5
#include "shaders\VS5.h"
#undef   dwVertexShader

#define  dwVertexShader dwVertexShaderVS6
#include "shaders\VS6.h"
#undef   dwVertexShader

#define  dwVertexShader dwVertexShaderVS7
#include "shaders\VS7.h"
#undef   dwVertexShader
...
RwUInt32 VSRender::VertexShaders[8];
#ifdef DEVBUILD
RwUInt32 VSRender::VertexShadersDebug[8];
#endif
...
{
#ifdef DEVBUILD
	if ( Settings::GetInstance()->bDebugNoModulate )
	{
		if ( (flags & rxGEOMETRY_MODULATE)!=0 )
			return;
	}

	if ( Settings::GetInstance()->bDebugNoDiffuse )
	{
		if ( (flags & rxGEOMETRY_PRELIT)!=0 )
			return;
	}

	if ( Settings::GetInstance()->bDebugNoNormals )
	{
		if ( (flags & rxGEOMETRY_NORMALS)!=0 )
			return;
	}
#endif
	...
	* Vertex shader
	*/
	RwInt32 shaderid = ((flags & rxGEOMETRY_PRELIT)!= 0) * 1 + ((flags & rxGEOMETRY_NORMALS)!= 0) * 2 + ((flags & rxGEOMETRY_MODULATE)!= 0) * 4;
#ifdef DEVBUILD
